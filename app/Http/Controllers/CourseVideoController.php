...
class CourseVideoController extends Controller
{
    ...
    public function update(StoreCourseVideoRequest $request, CourseVideo $courseVideo)
    {
        ...
        return redirect()->route('admin.courses.show', parameters: $courseVideo->course_id)->with('success', 'Category updated successfully');
    }
    ...
    public function destroy(CourseVideo $courseVideo )
    {
        ...
        try {
            $courseVideo->delete();
            DB::commit();
            return redirect()->route('admin.courses.show', $courseVideo->course_id)->with('success', 'Category deleted successfully');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->route('admin.courses.show', $courseVideo->course_id)->with('error', 'Failed to delete category: ' . $e->getMessage());
        } catch (\Throwable $th) {
            //throw $th;
        }
    }
